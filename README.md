...
話者条件は **SPEAKER_EMBED に一本化**されている。`IrodoriTextEncode` の `speaker_embed` 入力に、
以下のいずれかを接続する（未接続なら話者参照なし）:

- **参照音声から**: `LoadAudio → VAEEncodeAudio → IrodoriSpeakerEncode`（VoiceClone。トークン数は参照長に比例、約25/秒）
- **学習済み inversion 埋め込み**: `IrodoriSpeakerEmbedLoader`（16トークン等の固定長）
- **複数のブレンド**: `IrodoriSpeakerEmbedMerge`

`speaker_embed` は `IrodoriEmptyLatent` にも接続でき、duration 予測の精度が上がる。
...
> 補足: joint-softmax の attention 質量は各話者ソースのトークン数にほぼ比例する。
> 参照音声埋め込み（約25トークン/秒）は inversion 埋め込み（16等）より件数が多く、
> concat すると参照側が支配的になりやすい。件数が効き目に直結するので、ブレンド比は
> 主に **どのソースを何トークン与えるか**で決まる（Merge の concat 件数）。
...
[LoadAudio] → [VAEEncodeAudio] → [IrodoriSpeakerEncode] → SPEAKER_EMBED   ← VoiceClone時
[IrodoriSpeakerEmbedLoader] → SPEAKER_EMBED                              ← inversion埋め込み使用時

[IrodoriTextEncode] MODEL + text + SPEAKER_EMBED + caption
